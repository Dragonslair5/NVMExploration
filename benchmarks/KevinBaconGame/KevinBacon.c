...
node* newNode(char *name){
  ...
  np->name = malloc( (strlen(name) + 2) * sizeof(char)); // +2 for \0 and safeness
  strcpy(np->name, name);
  np->link = newEdge();
  return np;
}
...
pStruct* buildTheGraph(int argc, char **argv){
  ...
  pRoot = malloc(sizeof(pStruct));
  pRoot->savedColorThreshold = colorThreshold;
  listOfActors = calloc(1, HASH_DOMAIN * sizeof(node*));
  listOfMovies = calloc(1, HASH_DOMAIN * sizeof(node*));
